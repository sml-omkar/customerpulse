...
exports.cxoDashboardController = void 0;
const database_1 = require("../lib/database");
const errorHandler_1 = require("../middleware/errorHandler");
const client_1 = require("../generated/prisma/client");
exports.cxoDashboardController = {
    ...
    async getTeam(req, res) {
        ...
        const cxoDepartments = await database_1.prisma.department.findMany({
            where: { cxoId },
            select: { id: true, name: true },
        });
        if (cxoDepartments.length === 0) {
            throw new errorHandler_1.AppError("No departments are assigned to you", 400);
        }
        const cxoDeptIds = cxoDepartments.map((d) => d.id);
        if (departmentId && !cxoDeptIds.includes(departmentId)) {
            throw new errorHandler_1.AppError("Department is not under your management", 403);
        }
    ...
    async getUserTickets(req, res) {
        ...
        if (!targetUser ||
            ...
            throw new errorHandler_1.AppError("User is not in a department under your management", 403);
        }
        const tickets = await database_1.prisma.ticket.findMany({
            where: {
    ...
    async reassignTicket(req, res) {
        ...
        if (!cxoDeptIds.includes(ticket.departmentId)) {
            throw new errorHandler_1.AppError("Ticket is not in a department under your management", 403);
        }
        ...
        if (ticket.requesterId === newAssigneeId) {
            throw new errorHandler_1.AppError("Cannot reassign a ticket to the agent who raised it", 400);
        }
        ...
        if (newAssignee.agentsdepartmentId !== ticket.departmentId) {
            throw new errorHandler_1.AppError("New assignee must belong to the same department as the ticket", 400);
        }
        const newAssigneeActiveCount = await database_1.prisma.ticket.count({
            where: { assigneeId: newAssigneeId, status: { not: "RESOLVED" } },
        });
        if (newAssigneeActiveCount > 3) {
            throw new errorHandler_1.AppError("This agent already has more than 3 active tickets", 400);
        }
    ...
    async getDepartmentOverview(req, res) {
        ...
        if (user?.role !== client_1.UserRole.CXO) {
            throw new errorHandler_1.AppError("Only CXO can access this endpoint", 403);
        }
        const departments = await database_1.prisma.department.findMany({
            where: { cxoId },
            include: {
        ...
        const departmentsWithMetrics = await Promise.all(departments
            .filter(d => targetDeptIds.includes(d.id))
            .map(async (dept) => {
            const [totalTickets, openTickets, inProgressTickets, resolvedTickets, breachedTickets, onHoldTickets] = await Promise.all([
                database_1.prisma.ticket.count({ where: { departmentId: dept.id } }),
                database_1.prisma.ticket.count({ where: { departmentId: dept.id, status: "OPEN" } }),
                database_1.prisma.ticket.count({ where: { departmentId: dept.id, status: "IN_PROGRESS" } }),
                database_1.prisma.ticket.count({ where: { departmentId: dept.id, status: "RESOLVED" } }),
                database_1.prisma.ticket.count({ where: { departmentId: dept.id, slaBreached: true, status: { not: "RESOLVED" } } }),
                database_1.prisma.ticket.count({ where: { departmentId: dept.id, status: "ON_HOLD" } }),
            ]);
            ...
            const tatData = await database_1.prisma.ticketStatusHistory.aggregate({
        ...
        const summary = {
            ...
            avgSlaComplianceRate: Math.round(departmentsWithMetrics.reduce((sum, d) => sum + d.metrics.slaComplianceRate, 0) /
    ...
    async getManagerPerformance(req, res) {
        ...
        let departments = await database_1.prisma.department.findMany({
            where: { cxoId },
        ...
        if (departmentId) {
            ...
            if (departments.length === 0) {
                throw new errorHandler_1.AppError("Department not found under your management", 404);
            }
        }
        const managerPerformance = await Promise.all(departments
            ...
            const [totalTickets, openTickets, resolvedTickets, breachedTickets, inProgressTickets, onHoldTickets,] = await Promise.all([
                database_1.prisma.ticket.count({ where: { departmentId: dept.id } }),
                database_1.prisma.ticket.count({ where: { departmentId: dept.id, status: "OPEN" } }),
                database_1.prisma.ticket.count({ where: { departmentId: dept.id, status: "RESOLVED" } }),
                database_1.prisma.ticket.count({ where: { departmentId: dept.id, slaBreached: true, status: { not: "RESOLVED" } } }),
    ...
    async getAgentPerformance(req, res) {
        ...
        const agentMetrics = await Promise.all(agents.map(async (agent) => {
            const [activeTickets, resolvedTickets, breachedTickets, openTickets, inProgressTickets, onHoldTickets, escalatedTickets,] = await Promise.all([
                database_1.prisma.ticket.count({
    ...
    async getRiskMetrics(req, res) {
        ...
        const deptIds = departments.map(d => d.id);
        const breachedTickets = await database_1.prisma.ticket.findMany({
            where: {
        ...
        const atRiskTickets = await database_1.prisma.ticket.findMany({
            where: {
                departmentId: { in: deptIds },
                priority: { in: ["P1", "P2"] },
    ...
    async getDepartmentComparison(req, res) {
        ...
        const comparison = await Promise.all(departments.map(async (dept) => {
            const [p1, p2, p3, p4, open, inProgress, onHold, resolved, breached] = await Promise.all([
                database_1.prisma.ticket.count({ where: { departmentId: dept.id, priority: "P1" } }),
                database_1.prisma.ticket.count({ where: { departmentId: dept.id, priority: "P2" } }),
                database_1.prisma.ticket.count({ where: { departmentId: dept.id, priority: "P3" } }),
                database_1.prisma.ticket.count({ where: { departmentId: dept.id, priority: "P4" } }),
                database_1.prisma.ticket.count({ where: { departmentId: dept.id, status: "OPEN" } }),
                database_1.prisma.ticket.count({ where: { departmentId: dept.id, status: "IN_PROGRESS" } }),
                database_1.prisma.ticket.count({ where: { departmentId: dept.id, status: "ON_HOLD" } }),
                database_1.prisma.ticket.count({ where: { departmentId: dept.id, status: "RESOLVED" } }),
...
//# sourceMappingURL=cxodashboard.controller.js.map